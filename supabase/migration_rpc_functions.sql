...
BEGIN
  -- Validar e travar registro do policial
  SELECT * INTO v_policial FROM usuarios WHERE matricula = (p_cautela->>'matricula_policial') AND deletado_em IS NULL FOR UPDATE;
  IF NOT FOUND THEN
  ...
  -- Processar cada item da cautela atomicamente com trava de linha (SELECT FOR UPDATE)
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_itens)
  LOOP
    v_qty_solicitada := COALESCE((v_item->>'quantidade')::integer, 1);

    SELECT * INTO v_material FROM materiais WHERE id_material = (v_item->>'id_material') AND deletado_em IS NULL FOR UPDATE;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Material % não encontrado.', (v_item->>'id_material');
    END IF;

    IF v_material.controle_quantidade THEN
      IF COALESCE(v_material.quantidade, 0) < v_qty_solicitada THEN
        RAISE EXCEPTION 'Estoque insuficiente para o material % (disponível: %, solicitado: %).', v_material.modelo, v_material.quantidade, v_qty_solicitada;
      END IF;

      UPDATE materiais 
      SET quantidade = quantidade - v_qty_solicitada 
...
BEGIN
  -- Travar e carregar cautela
  SELECT * INTO v_cautela FROM cautelas WHERE id_cautela = p_id_cautela FOR UPDATE;
  IF NOT FOUND THEN
  ...
  LOOP
    ...
    SELECT * INTO v_material FROM materiais WHERE id_material = (v_item_devolvido->>'id_material') FOR UPDATE;
    IF FOUND THEN
      IF v_material.controle_quantidade THEN
        IF NOT COALESCE((v_item_devolvido->>'consumido')::boolean, false) THEN
          UPDATE materiais SET quantidade = quantidade + v_qty_devolvida WHERE id_material = v_material.id_material;
        END IF;
      ELSE
