...
async function runTest() {
  ...
  try {
    ...
    const { error: updMatErr } = await supabase.from('materiais').update({ status_atual: 'cautelado' }).eq('id_material', testMaterialId);
    if (updMatErr) throw new Error(`Erro ao atualizar status do material para cautelado: ${updMatErr.message}`);

    const { error: updUserErr } = await supabase.from('usuarios').update({ situacao_cautela: 'pendente_devolucao' }).eq('matricula', testMatricula);
    if (updUserErr) throw new Error(`Erro ao atualizar situacao_cautela do policial para pendente_devolucao: ${updUserErr.message}`);
    ...
    const { error: updMatDevErr } = await supabase.from('materiais').update({ status_atual: 'disponivel' }).eq('id_material', testMaterialId);
    if (updMatDevErr) throw new Error(`Erro ao atualizar status do material para disponivel: ${updMatDevErr.message}`);

    const { error: updUserDevErr } = await supabase.from('usuarios').update({ situacao_cautela: 'apto' }).eq('matricula', testMatricula);
    if (updUserDevErr) throw new Error(`Erro ao atualizar situacao_cautela do policial para apto: ${updUserDevErr.message}`);
