...
const rawInput = `
colete SC00001230400140005971
colete SC00001230400140005431
colete SC00001230400140007601
colete SC00001230400140002011
colete SC00001230400140002021
colete SC00001230400140005511
colete SC0000xxxxxxxxxxxxx5871
colete SC00001230400140005821
colete SC00001230400140006601
colete SC00001230400140005481
colete SCxxxxxxxxxxxxxxxxxxxx
colete SC00001230400140005931
colete SC00001230400140005501
colete SC00001230400140005491
colete SC0000123040014000762
`;
...
for (const line of lines) {
  const match = line.match(/^colete\s+(SC.+)$/i);
  if (match) {
    const sn = match[1]; // ex: SC00001230400140005971
    const modelo = 'Colete Protecop Tam G';
    if (!seen.has(sn)) {
      seen.add(sn);
...
console.log('Total de coletes Protecop extraídos:', items.length);
...
console.log('Novos coletes adicionados ao backup_reserva_armamento.json:', addedCount);
...
if (mockContent.includes('export const mockMateriais: Material[] = [')) {
  if (!mockContent.includes(`id_material: '${items[0].sn}'`)) {
    ...
    console.log('mockData.ts atualizado com 15 coletes Protecop!');
  } else {
    console.log('mockData.ts já continha os coletes.');
  }
}

// 3. Gerar coletes_protecop_insert.sql
let sql = `-- ====================================================================\n`;
sql += `-- INSERÇÃO DOS 15 COLETES PROTECOP (TAM G) NA TABELA 'materiais'\n`;
...
const valuesStr = items.map(item => 
  `  ('${item.sn}', 'CAT-MANUTENCAO', '${item.modelo}', 'PROTECOP', '', 'Tamanho G', 'disponivel', '2026-07-23', false, 1, '${rpmonId}')`
).join(',\n');
...
fs.writeFileSync(path.join(__dirname, '..', 'coletes_protecop_insert.sql'), sql, 'utf8');
console.log('coletes_protecop_insert.sql gerado com sucesso!');
